...
class Corredor(Thread):
    ...
    def avanzar(self) -> None:
        avance = randint(Corredor.PORCENTAJE_MIN, Corredor.PORCENTAJE_MAX)/100
        self.posicion += (avance * self.velocidad)
    ...
    def intentar_capturar_tortuga(self) -> None:
        adquerido = self.lock_tortuga.acquire(blocking=False)
        if adquerido:
    ...
    def correr_segunda_mitad(self) -> bool:
        while self.__correr:
            ...
            with self.lock_verificar_tortuga:
                ...
                if self.posicion >= 100 and self.tiene_tortuga:
                    self.senal_fin.set()
                    self.lock_tortuga.release()
                    return True
                    
                if not self.tiene_tortuga:
                    self.robar_tortuga()
